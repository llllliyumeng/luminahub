...
# Luminahub 平台 - 知识获取与分享社区
后端 & 前端开发（前端采用 AI 辅助开发）
- **后端地址**：`https://github.com/<your-github-username>/luminahub_be`
- **前端地址**：`https://github.com/<your-github-username>/luminahub_fe`
...
- **项目细节与亮点**：
    ...
    - **AI 问答系统**：开发 Luminahub 平台 RAG 知识问答系统，实现用户调用接口→索引检查→向量检索→Prompt 构造→大模型流式生成的全流程，通过合理分块、幂等删除保持单一版本、预索引减少首次提问等待时间等，显著提升用户围绕单篇知文的智能问答效率与准确性。
